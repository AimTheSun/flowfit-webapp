package com.flowfit.backendflowfit.services;

import com.flowfit.backendflowfit.model.Client;
import com.flowfit.backendflowfit.model.PointsCooldown;
import com.flowfit.backendflowfit.repository.ClientRepository;
import com.flowfit.backendflowfit.repository.PointsCooldownRepository;
...
public class ClientService {
    ...
    public List<Client> getClientsInGym() {
        return clientRepository.findByInsideGym(Client.ClientStatusInGym.INSIDEGYM);
    }

    public void changeClientStatusInGym(Integer clientId, Client.ClientStatusInGym status) {
        Client client = getClientById(clientId);

        if (client!= null) {
            client.setInsideGym(status);
            clientRepository.save(client);
        } else {
            throw new RuntimeException("Client not found with id: " + clientId);
        }
